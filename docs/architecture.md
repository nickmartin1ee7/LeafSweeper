...
touch events → Main._UnhandledInput ─┬→ Sweeper (flings debris) → onSwipeCompleted → LevelStats.CountSwipe
                                     ...
                                     ├→ two dead taps inside the double-tap window → Sweeper.Burst (radial fling, same 12-piece cap → onSwipeCompleted)
