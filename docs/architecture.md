...
WinLevel → LevelStats.Stop → SaveData.RecordClear → Bug.Celebrate → CelebrationFinished → Hud.ShowWin(comment, stats)
         ...
         └→ next round is a storm round → StormWarn.ShowWarning (electrical "Storm Round" sign; it lingers 2s into the storm round's opening, then fades out over 1s)
