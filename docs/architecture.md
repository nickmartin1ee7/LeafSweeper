...
| `scripts/MainMenu.cs` | Title screen: Play (resume), New game (shown only after ≥1 clear), lifetime progress (levels, time, gusts) + favorite critter, and a tappable "🌐 Update Available (vX.Y.Z)" line that appears only when a newer GitHub release exists — a `LinkButton` whose `Uri` opens the releases page in the browser. |
| `scripts/UpdateChecker.cs` | Update notification: once at boot it GETs the GitHub releases API for the repo's latest release (`tag_name`, `User-Agent` header, 10s timeout), compares it against the installed `config/version` from `project.godot` (`System.Version` semantics, `v` prefix and prerelease suffix tolerated) and raises `UpdateAvailable` when newer — `MainMenu` then reveals the update line. Every failure path (offline, HTTP error, rate limit, unreadable tag) is silent; diagnostics print with an `UPDATE` prefix for headless/logcat greps. `LEAF_FAKE_UPDATE=<tag>` skips the network for testing. |
