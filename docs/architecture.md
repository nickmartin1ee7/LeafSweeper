...
| `scripts/MainMenu.cs` | Title screen: Play (resume), New game (shown only after ≥1 clear), lifetime progress (levels, time, gusts) + favorite critter, and a version line under the buttons: gold, tappable "🌐 Update Available (vX.Y.Z)" (a `LinkButton` whose `Uri` opens the releases page) when a newer GitHub release exists, otherwise a muted, inert "vX.Y.Z" showing the installed version. |
| `scripts/UpdateChecker.cs` | Update notification: once at boot it GETs the GitHub releases API for the repo's latest release (`tag_name`, `User-Agent` header, 10s timeout), compares it against the installed `config/version` from `project.godot` (`System.Version` semantics, `v` prefix and prerelease suffix tolerated) and raises `UpdateAvailable` when newer or `UpToDate` with the installed version otherwise — `MainMenu` shows the matching line either way. Every failure path (offline, HTTP error, rate limit, unreadable tag) is silent; diagnostics print with an `UPDATE` prefix for headless/logcat greps. `LEAF_FAKE_UPDATE=<tag>` skips the network for testing. |
