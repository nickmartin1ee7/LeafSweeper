...
|--------|----------------|
| `scripts/Main.cs` | Controller: builds tree, state machine, level setup/teardown, input routing, win flow, debris spawn layout, wind gust (clears ~10% of remaining debris with streak effects), restart handling, petal sparkle. Also hosts the `LEAF_AUTOPLAY=1` headless self-test. |
...
| `scripts/Hud.cs` | In-game UI: top bar (level, swipes, Gust + Restart icon buttons), win overlay (title, **bug seat slot** the celebrated bug is reparented into, comment, stats, Next/Menu) and a Restart confirmation dialog. Static `MakeLabel`/`MakeButton` helpers shared with the menu. |
