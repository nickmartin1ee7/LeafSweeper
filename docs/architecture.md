...
| `scripts/RoundConfig.cs` | Difficulty curves saturating at level 200: `Coverage` (debris density as floor-area fraction), `BugScale`, `Camouflage`, `GustCoinsPerRound` (the one gust coin hidden each round — the same count on every round type, so storm difficulty never rides the coin economy). Pure functions — easy to tune. |
...
StartLevel → SpawnDebris (SettleIn tumble-drop, diagonal stagger) → [touches locked] → OnSettleFinished → bug + gust coin hidden at random spots under the settled debris, round clock starts
