...
2. **Drag** your finger across the screen to sweep debris away. Heavy things
...
4. Tap **Next** to keep going. Difficulty rises *very* gently: by level 200
   there are ~90 pieces of debris instead of ~40, the bug is slightly smaller
   and may blend in a little — that's all.

Eight bug types hide in the leaves: ladybug, butterfly, centipede, moth,
grasshopper, dragonfly, beetle and snail — each with its own size and
silhouette, so you learn to spot shapes rather than colors.
...
LEAF_AUTOPLAY=1 godot --headless --quit-after 300   # self-test: plays a level, verifies save round-trip, exit code 0 on pass
```
